...
## Windows C++ desktop UI scaffold

`src/winui/TurtleClass.WindowsDesktop` contains a Windows-only native C++ desktop UI scaffold. It is not included in the cross-platform CMake build because it requires Windows, the Windows SDK, and MSVC/Visual Studio. The scaffold is intentionally UI-shell only: it displays placeholder student state and queues command intent without calculating points, levels, or badges in UI code. It is not a C# WinUI project.
